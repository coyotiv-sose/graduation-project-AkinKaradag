...
describe('Vehicle', () => {
  ...
  it('should not create a vehicle, when a required field is empty', async () => {
    const response = await request(app).post(`/companies/${company.body._id}/vehicles`).send({
      brand: 'Mercedes',
      model: 'Sprinter',
      year: 2005,
    })
  ...
  it('should find all vehicles of a comapny', async () => {
    const vehicle1 = await request(app).post(`/companies/${company.body._id}/vehicles`).send({
      name: 'Truck22',
      brand: 'Mercedes',
      model: 'Sprinter',
      year: 2009,
    ...
    const vehicle2 = await request(app).post(`/companies/${company.body._id}/vehicles`).send({
      name: 'Truck23',
      brand: 'MAN',
      model: 'Cargo',
      year: 2009,
    ...
    const response = await request(app).get(`/companies/${company.body._id}/vehicles`)
    expect(response.status).toBe(200)
  ...
  it('should not find vehicles if there is a invalid company Id', async() => {
    const response = await request(app).get('/companies/invalid-id/vehicles')
    expect(response.status).toBe(500)
  })
